...
import { faLock, faInfoCircle, faEnvelope } from '@fortawesome/free-solid-svg-icons';
...
const LoginForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    .catch(function (err) {
      ...
      } else if (err.response?.status === 422 || err?.response?.data?.messageCode === 3 || err?.response?.data?.messageCode === 2 || err?.response?.data?.messageCode === 0) {
  ...
  return (
    <div className='form-box'>
      <form onSubmit={handleSubmit} className='form-auth'>
        ...
        <div className='form-auth__text text-start'>
          <p ref={errRef} className={"text-danger m-0 " + (errMsg ? "errmsg" : "offscreen")} aria-live="assertive">{errMsg}</p>
        </div>
        <div className='form-auth__text text-end'>
          <a href="#" className="text-link mx-2 my-0">Quên mật khẩu ?</a>
        </div>
        <button disabled={loading} className="btn btn-primary rounded-pill btn--form">
          {
            loading ? (
              <React.Fragment>
        ...
        <div className='social-form'>
          <p className="social-form__text">Hoặc đăng nhập bằng phương thức khác</p>
          <div className='social-form__media'>
            <a href="#" className='social-form__link'>
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" className="social-form__icon"><path d="M279.14 288l14.22-92.66h-88.91v-60.13c0-25.35 12.42-50.06 52.24-50.06h40.42V6.26S260.43 0 225.36 0c-73.22 0-121.08 44.38-121.08 124.72v70.62H22.89V288h81.39v224h100.17V288z"/></svg>
            </a>
            <a href="#" className='social-form__link'>
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 488 512" className="social-form__icon"><path d="M488 261.8C488 403.3 391.1 504 248 504 110.8 504 0 393.2 0 256S110.8 8 248 8c66.8 0 123 24.5 166.3 64.9l-67.5 64.9C258.5 52.6 94.3 116.6 94.3 256c0 86.5 69.1 156.6 153.7 156.6 98.2 0 135-70.4 140.8-106.9H248v-85.3h236.1c2.3 12.7 3.9 24.9 3.9 41.4z"/></svg>
            </a>
          </div>
        </div>
